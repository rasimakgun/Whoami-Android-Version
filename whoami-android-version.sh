...
echo -n ${RED}Your old ip: && echo -n ${green} &&  curl ifconfig.co
echo -n ${RED}Your Whoami ip:&& echo -n ${green}  &&  proxychains curl ifconfig.me
echo ""
echo -n ${RED}WARNING: && echo ${green}"If you want to clear the terminal just write any number(ex:1,2,3)"
...
if n=0; [[ $n -lt $input ]]; then
      bash n=0; while [[ $n -lt $input ]]; do gnome-terminal --title="WHO AM I TERMİNAL"  --  bash .term.sh  ; ((n= n+1)); done && clear && echo "${green}##################################### Terminal has been cleaned #####################################"
elif  [[ : ]]; then
     proxychains $input
fi
done
